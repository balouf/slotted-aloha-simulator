...
@njit
def core_rec(c_min=3, c_max=10, n=64, t_sim=20):
    base = 2**c_min
    state = np.zeros(n, dtype=np.int64)
    countdown = np.floor(base*np.random.rand(n)).astype(np.int64)
    states = np.zeros((c_max - c_min + 1, t_sim + 1), dtype=np.int64)
    ...
    for t in range(t_sim + 1):
        ...
        e = 0
        emitter = 0
        d_t = np.zeros(c_max - c_min + 1, dtype=np.int64)
...
@njit
def rec_run(c_min=3, c_max=10, n=64, t_sim=20, m=10, seed=None):
    if seed is not None:
        np.random.seed(seed)
    # Main run
